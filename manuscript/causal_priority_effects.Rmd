...
In this paper, we use causal mediation analysis on a experiment wherein salicylic acid (SA) and jasmonic acid (JA) exposures were used to modify the timing of the pathogen arrival. A main interest is in the effect of pathogen arrival on the [end of study outcome]. Pathogen arrival itself was not randomized, but it was mediated by the experimental treatments. We show how causal inference methods clarify assumptions regarding inferences for mediated effects. We also show how approaches to mediation such as structural equation models make certain implicit assumptions, which, if violated, may lead to incorrect conclusions. 
