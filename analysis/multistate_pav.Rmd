...
temp <- output %>% 
  tidyr::gather(key = "metric", value = "value", -comparison, -t, -PAV, -lpC) %>%
  mutate(
    covariate = paste("PAV:", PAV, "lpC:", lpC)
  )


ggplot(
    data = temp %>% filter(t <= 21),
    aes(x = t, y = value, color = covariate)
  ) + 
